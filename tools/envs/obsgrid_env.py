...
class ObsGrid(Env):
    ...
    def step(self, action):
        ...
        #if self.done:
        #    return None
        ns = self.sample(self.state, action)
        r = self.R[self.state, action, ns]
        self.state = ns
    ...
    def state_to_pos(self, state):
        ...
        """
        return divmod(int(state), int(self.cols))
    ...
    # Added for the UCT exercise:

    def Applicable(self, state):
        return [a for a in range(len(self.actions))] if state not in self.S_g() else []
    
    def get_cost(self, state, action, next_state):
        return self.RS[next_state]

    def S_g(self):
        terminal_states = []
        for state in self.staterange:
